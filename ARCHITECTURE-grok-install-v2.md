...
| Palette (4-color system; max 3 per asset per D9) | `#0A0A0A` (background/text), `#00F0FF` (primary accent), `#00FF9D` (secondary accent), `#FF2D55` (experimental accent — reserved for xlOS only per ARCHITECTURE-xlOS.md section 5) | grok-install-action/grok-install-brand/tokens/colors.css canonical palette |
...
- No orchestration framework, debate runtimes, or research surfaces. The whole `grok_orchestra/` Python package and its dependencies live in xlOS per Q1 + Q2 resolutions; this repo does not depend on xlOS at runtime.

## 8. Open questions — RESOLVED 2026-05-11

All 6 questions resolved before merge. Resolutions reflected inline in sections 2, 3, 5, 7. This section preserved as record.

1. **Q1 — `combined.py` target → MOVED to xlOS.** The combined Bridge+Orchestra runner lives in `xlOS/orchestration/combined.py`. The standalone Bridge pipeline (`safety_audit/`, `builder/`, `deploy/`) stays in this repo and is self-contained. Stable repo never depends on experimental repo at runtime.
2. **Q2 — Orchestra "infrastructure" files → MOVED to xlOS as a unit.** The whole `grok_orchestra/` Python package (cli.py, _roles.py, _errors.py, _templates.py, sources/, tracing/, plus combined.py from Q1) goes to xlOS. Only `safety_audit/` Lucas-veto remains the cross-cutting piece, and that gets the dedicated `xlOS/safety/lucas-veto/` placement per D2.
3. **Q3 — Cloudflare Worker placement → ROOT `/worker/`.** Peer to `cli/`, `marketplace/`, `ide/`. The Worker backs auth, dashboard, profile, mascot, safety, and manifest mint endpoints — not just the marketplace surface — so colocation under `marketplace/worker/` would mislead.
4. **Q4 — Streamlit finance dashboards → SUB-APPS with CLI passthrough + `finance-app` certification badge.** The 4 X-Money apps in `agents/finance/` ship with a parallel `streamlit run` story documented in their READMEs and flagged in the marketplace via a distinct `finance-app` certification badge. The CLI adds a soft passthrough: `grok-install run <finance-agent>` detects the Streamlit launcher and execs it, giving users one mental model while the runtime branches internally. Phase 2b implements the detection logic.
5. **Q5 — Brand palette → CANONICAL TRIO + `#FF2D55` reserved.** Canonical lineage wins (`#0A0A0A` + `#00F0FF` + `#00FF9D` per grok-install-brand/colors.css). `#FF2D55` is the 4th color in the system, reserved for xlOS experimental markers only per ARCHITECTURE-xlOS.md section 5. Marketplace's "Plasma/Aurora" pair is dropped per D9.
6. **Q6 — `bridge.schema.json` → `spec/v2.14/schemas/bridge.schema.json`.** Single canonical source. IDE extension reads from `spec/`, not from a parallel copy under `ide/`. Carries the 1h "sync Draft 2020-12 + 6-target enum" fix flagged in PORTFOLIO-MAP.md.
